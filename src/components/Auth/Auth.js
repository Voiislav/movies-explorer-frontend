...
function Auth({ handleSubmit, handleChange, titleText, submitButtonText, clarificationText, linkText, linkPath, showNameInput }) {
  return (
    <section className='authorization'>
      ...
      {showNameInput && (
          <>
            ...
            <input onChange={handleChange} minLength={2} maxLength={30} type='text' className='authorization__input' id='name' placeholder='Введите имя' required></input>
          </>
        )}
        <label htmlFor='email' className='authorization__label'>E-mail</label>
        <input onChange={handleChange} type='email' className='authorization__input' id='email' placeholder='Введите адрес электронной почты' required></input>
        <label htmlFor='password' className='authorization__label'>Пароль</label>
        <input onChange={handleChange} type='password' className='authorization__input' id='password' placeholder='Введите пароль' required></input>
